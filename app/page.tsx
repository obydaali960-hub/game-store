...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-white font-sans selection:bg-cyan-500 selection:text-black">
      ...
      <div className="overflow-hidden py-4 bg-[#080c14] border-y border-gray-800/40 relative">
        <div className="flex gap-6 animate-marquee whitespace-nowrap px-4">
          {gamesList.concat(gamesList).map((gameName, index) => (
            <div 
              key={`${gameName}-${index}`}
              className="inline-flex items-center gap-3 bg-[#131b2e] border border-gray-800/80 px-5 py-2.5 rounded-xl text-sm font-semibold text-gray-300 shadow-sm shrink-0 select-none pointer-events-none"
            >
